...
export function SkillsSection() {
  ...
  // 初始化气泡位置、呼吸缩放和颜色
  // 初始化气泡位置、呼吸缩放和颜色
  useEffect(() => {const initialPositions = {};
    const initialBreath = {};
    const initialColors = {};
  ...
  useEffect(() => {const animate = () => {
      ...
      setBubblePositions((prev) => {
        ...
        // 计算气泡间的斥力
        // 计算气泡间的斥力
        Object.keys(newPositions).forEach((skillId) => {const pos = newPositions[skillId];
          let forceX = 0;
          let forceY = 0;

          // 计算与其他气泡的斥力
          // 计算与其他气泡的斥力
          Object.keys(newPositions).forEach((otherId) => {if (skillId !== otherId) {
              ...
              // 如果距离太近，产生斥力
              // 如果距离太近，产生斥力
              if (distance < 20 && distance > 0) {// 斥力强度与距离成反比
          ...
          // 边界检测和反弹
          // 边界检测和反弹
          if (pos.x <= 5 || pos.x >= 95) {pos.vx = -pos.vx * 0.9;
          ...
          // 增加随机扰动
          // 增加随机扰动
          if (Math.random() < 0.02) {pos.vx += (Math.random() - 0.5) * 0.2;
            pos.vy += (Math.random() - 0.5) * 0.2;
          }

          // 添加正弦波动
          // 添加正弦波动
          pos.vx += Math.sin(time + parseInt(skillId)) * 0.01;pos.vy += Math.cos(time + parseInt(skillId)) * 0.01;

          // 限制速度范围
          // 限制速度范围
          pos.vx = Math.max(-0.8, Math.min(0.8, pos.vx));pos.vy = Math.max(-0.8, Math.min(0.8, pos.vy));
        });
        return newPositions;
      });
      ...
      // 呼吸效果动画
      // 呼吸效果动画
      setBreathScale((prev) => {const newBreath = {
          ...prev };
  ...
  return <section id="skills" className="py-20 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">我的技能点
        ...
        <div className="flex justify-center">
          <div ref={containerRef} className="relative bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl border-2 border-indigo-200 w-full max-w-6xl h-[600px] overflow-hidden">
            ...
            // 增大基础尺寸
            // 增大基础尺寸
            const hoverSize = 180; // 增大悬停尺寸
            // 增大悬停尺寸
            const currentSize = isHovered ? hoverSize : baseSize;const finalSize = currentSize * breath.scale;
